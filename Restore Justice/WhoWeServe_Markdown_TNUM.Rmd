...
counties <- as.data.frame(unlist(str_match(subj$subj,"county:\\s*(.*?)\\s*/")))
counties <- tibble(unique(counties[,2]))
names(counties)="counties"

data_tnum <- data.frame()

#length(counties$counties)
start <- Sys.time()
for (i in c(1:length(counties$counties))){
 t <- tnum.query(query=paste0("IDOC:2021:March* and subj:*",counties[i,],"*"),max=300001)
 t <- t %>%
   mutate(value=ifelse(t$`value-type`=="TsNumber",t$si_value$string,t$value)) %>%
   select(date,html_varname,server,author,subject,wrapper,property,id,`value-type`,value)
  data_tnum <- bind_rows(data_tnum, t)
}
